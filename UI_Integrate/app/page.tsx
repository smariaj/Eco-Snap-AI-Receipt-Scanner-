...
import { Leaf, Menu, X, Upload, BarChart3, Camera, PieChart, ShoppingCart, Lightbulb } from "lucide-react"
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        {showResults && (
          <section
            id="results"
            className="relative py-10 px-4 pt-36 min-h-screen animate-on-scroll overflow-hidden"
            ...
            <div className="container max-w-7xl mx-auto relative z-10">
              <div className="text-center mb-16">
                ...
                <p className="max-w-3xl mx-auto text-xl text-black">
                  Quickly see your footprint and tips to shrink it.
                </p>
              </div>
              ...
              {analysisResult ? (
                <>
                  ...
                  <div className="grid md:grid-cols-2 gap-10">
                    {/* Left: Total Carbon Footprint */}
                    <Card className="shadow-md rounded-xl bg-white">
                      <CardHeader className="pb-4">
                        <CardTitle className="flex items-center gap-3 text-2xl" style={{ color: "#004d40" }}>
                          <Leaf className="h-6 w-6" />
                          Your Total Carbon Footprint
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="text-5xl font-bold mb-2" style={{ color: "#004d40" }}>
                          {analysisResult.totalFootprint.toFixed(2)} kg CO₂e
                        </div>
                        <p className="text-gray-700">
                          This is equivalent to driving {(analysisResult.totalFootprint / 0.21).toFixed(0)} km in a car.
                        </p>
                      </CardContent>
                    </Card>

                    {/* Right: Impact by Category with Pie Chart */}
                    <Card className="shadow-md rounded-xl bg-white">
                      <CardHeader className="pb-4">
                        <CardTitle className="flex items-center gap-3 text-2xl" style={{ color: "#004d40" }}>
                      ...
                      <CardContent>
                        {(() => {
                          const palette = ["#2e7d32", "#556B2F", "#607D8B", "#004d40", "#8BC34A"]
                          const toSlug = (s: string) => s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "")
                          const config = Object.fromEntries(
                            analysisResult.categories.map((cat, i) => [
                              toSlug(cat.name),
                              { label: cat.name, color: palette[i % palette.length] },
                            ])
                          )
                          return (
                            <ChartContainer config={config} className="max-w-md mx-auto">
                              <Recharts.PieChart>
                                <Recharts.Pie
                                  data={analysisResult.categories.map((cat) => ({
                                    name: toSlug(cat.name),
                                    value: cat.value,
                                    fill: `var(--color-${toSlug(cat.name)})`,
                                  }))}
                                  dataKey="value"
                                  nameKey="name"
                                  innerRadius={60}
                        ...
                        <div className="mt-4 text-black text-sm flex flex-col items-center gap-1">
                          {analysisResult.categories.map((cat, i) => {
                            const toSlug = (s: string) => s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "")
                            const slug = toSlug(cat.name)
                            return (
                              <div key={cat.name} className="flex items-center gap-2">
                                <span className="h-2 w-2 rounded-full" style={{ backgroundColor: `var(--color-${slug})` }} />
                                <span>{cat.name}</span>
                                <span className="font-semibold" style={{ color: "#004d40" }}>
                                  — {analysisResult.totalFootprint > 0 ? ((cat.value / analysisResult.totalFootprint) * 100).toFixed(1) : "0.0"}%
                                </span>
                              </div>
                            )
                          })}
                  ...
                  <div className="grid md:grid-cols-2 gap-10 mt-10">
                    {/* Left: Top Impact Items */}
                    <Card className="shadow-md rounded-xl bg-white">
                      <CardHeader className="pb-2">
                        <CardTitle className="flex items-center gap-3 text-2xl" style={{ color: "#004d40" }}>
                          <ShoppingCart className="h-6 w-6" />
                          Your Top Impact Items
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        <h4 className="font-semibold text-gray-800 mb-3">These items contributed the most:</h4>
                        <ul className="space-y-2 text-gray-900">
                          {[...analysisResult.itemBreakdown]
                            .sort((a, b) => b.co2e - a.co2e)
                            .slice(0, 3)
                            .map((item, i) => (
                              <li key={i} className="list-disc ml-5">
                                {item.item.charAt(0).toUpperCase() + item.item.slice(1)}: {item.co2e.toFixed(2)} kg CO₂e
                              </li>
                            ))}
                        </ul>
                      </CardContent>
                    ...
                    <Card className="shadow-md rounded-xl bg-white">
                      ...
                      <CardContent>
                        <h4 className="font-semibold text-gray-800 mb-3">Simple ways to reduce your impact:</h4>
                        <ul className="space-y-2 text-gray-900">
                          <li className="list-disc ml-5">
                            For Dates and Erythritol, consider buying in bulk or looking for products with minimal packaging.
                          </li>
                          <li className="list-disc ml-5">
                  ...
                  <Card className="shadow-md rounded-xl bg-white mt-10">
                    <CardHeader className="pb-4">
                      <CardTitle className="flex items-center gap-3 text-2xl" style={{ color: "#004d40" }}>
